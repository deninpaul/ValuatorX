...
class _PinPainter extends CustomPainter {
  ...
  _PinPainter({required this.text, required this.color, required this.textColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final textPainter = TextPainter(
      text: TextSpan(text: text, style: TextStyle(fontSize: radius * 0.8, color: textColor)),
      textAlign: TextAlign.center,
      textDirection: TextDirection.ltr,
    );

    textPainter.layout();
    final textOffset = Offset(center.dx - textPainter.width / 2, center.dy - textPainter.height / 2 + 2);

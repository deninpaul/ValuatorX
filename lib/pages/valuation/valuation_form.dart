...
import 'package:valuatorx/pages/common/draft_dialog.dart';
...
class _ValuationFormState extends State<ValuationForm> with TickerProviderStateMixin {
  ...
  populateForm() async {
    ...
    controllers[Valuation.DATE_OF_INSPECTION]!.text = DateFormat('dd/MM/yyyy').format(DateTime.now());
    if (widget.editMode) {
      ...
      if (await provider.draftExists(valuationToEdit)) {
        setState(() => showDraftDialog = true);
      }
    }
    setState(() => ready = true);
  }
  ...
  saveAsDraft() async {
    ...
    final base = generateValuation(draftId);
    timer = Timer.periodic(const Duration(seconds: 5), (_) async {
      final valuation = generateValuation(draftId, status: "Draft");
      if (!equal(base.toJson(), valuation.toJson())) {
